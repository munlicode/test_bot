...
required_version = version.parse("1.47.1")
current_version = version.parse(openai.__version__)
OPENAI_API_KEY = os.environ['OPENAI_API_KEY']
if current_version < required_version:
  raise ValueError(
      f"Error: OpenAI version {openai.__version__} is less than the required version 1.47.1"
  )
else:
    ...
    if run_status.status == 'requires_action':
      ...
      for tool_call in run_status.required_action.submit_tool_outputs.tool_calls:
        if tool_call.function.name == "create_lead":
          ...
          output = functions.create_lead(arguments["name"], arguments["phone"], arguments["order"], arguments["address"], arguments["order_time"])
